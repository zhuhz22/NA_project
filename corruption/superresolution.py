...
class SRConv(H_functions):
    ...
    def __init__(self, kernel, channels, img_dim, device, stride=1):
        ...
        self._singulars = torch.matmul(
            self.singulars_small.reshape(small_dim, 1), self.singulars_small.reshape(1, small_dim)
        ).reshape(small_dim**2)
        # permutation for matching the singular values. See P_1 in Appendix D.5.
        self._perm = (
            torch.Tensor(
                [self.img_dim * i + j for i in range(self.small_dim) for j in range(self.small_dim)]
...
def build_sr_bicubic(factor, device, image_size, data_channels=3):
    ...
    H_funcs = SRConv(kernel / kernel.sum(), data_channels, image_size, device, stride=factor)
