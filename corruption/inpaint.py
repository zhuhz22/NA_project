...
def load_freeform_masks(op_type):
    data_dir = Path("assets/datasets/ImageNet")
...
def build_inpaint_center(mask_type, image_size):
    assert mask_type == "center"

    # log.info(f"[Corrupt] Inpaint: {mask_type=}  ...")

    center_mask = get_center_mask([image_size, image_size])  # [1,1,256,256]
    ...
    def inpaint_center(img):
        ...
        return img * (1.0 - mask) + mask * 0, mask
...
def build_inpaint_freeform(mask_type):
    ...
    def inpaint_freeform(img):
        ...
        index = np.random.randint(n_freeform_masks)
